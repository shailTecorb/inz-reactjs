...
const VisaTypeSetting = ({ styles, visa_type_tab, activeTab, active }) => {
  ...
  const getAllCountryList = async () => {
    ...
    if (response.code == 200) {
      let filterCountry = response?.result?.countries?.filter((data) => data.select)
  ...
  let FilterVisaType = allVisaList?.filter((flterData) => {
    if (isVisaSearch == "") {
      ...
      flterData?.visaType?.toLowerCase().includes(isVisaSearch.toLowerCase())
    ) {
      return flterData;
    }
  });
        ...
        <div className={`form-group ${styles.fromGroup}`}>
          <div className={styles.selectinputBox}>
            <ul style={{ listStyle: "none" }}>
              <li style={{ cursor: "pointer" }} onClick={() => { handleList(item.countryId?._id) }}>{item?.countryId?.countryName + " " + ">"} </li>
            </ul>
          </div>
        </div>
      )
  ...
  const handleList = async (id) => {
    ...
    const response = await businesOwnerVisaList(id)
    if (response.code == 200){
